...
class ScheduleItemsTab(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        self.schedule_tree = ttk.Treeview(self, columns=("description", "quantity", "unit", "unit_rate", "total_cost", "variation"), show="tree headings")
        ...
        self.schedule_tree.heading("variation", text="Variation")
        ...
        self.schedule_tree.column("variation", width=90, stretch=tk.NO, anchor=tk.CENTER)
    ...
    def _load_schedule_items(self):
        ...
        def insert_item_recursive(items_list, parent_iid="", parent_sr_prefix="", level=0):
            ...
            for item in items_list:
                ...
                # Get unit rate for the current item and reference firm
                reference_firm = self.reference_firm_var.get()
                ...
                self.schedule_tree.insert(parent_iid, tk.END, iid=item['item_id'], text=current_sr_no, values=(display_description, item['quantity'], item['unit'], utils_helpers.format_currency_inr(unit_rate), utils_helpers.format_currency_inr(total_cost), ""))
