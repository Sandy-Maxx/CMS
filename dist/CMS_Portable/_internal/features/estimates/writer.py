...
def write_work_name_row(worksheet, work_description_data, current_row):
    ...
    # Set the cell value
    cell_value = f"Name Of Work : {
        work_description_data.get(
            'description', '')}"
    ...
    for item in data:
        ...
        for header in COLUMN_HEADERS:
            ...
            if key == "labour_amount":
                # Qty column is 3rd (index 2), Labour rate in Rs. is 7th (index
                # 6)
                qty_col_idx = COLUMN_HEADERS.index("Qty") + 1
                labour_rate_col_idx = COLUMN_HEADERS.index(
                    "Labour rate in Rs.") + 1
                formula = f"={
                    get_column_letter(qty_col_idx)}{current_row}*{
                    get_column_letter(labour_rate_col_idx)}{current_row}"
                row_values.append(formula)
            elif key == "total_in_rs":
                # Qty column is 3rd (index 2), Rate in Rs is 4th (index 3),
                # Labour Amount is 8th (index 7)
                qty_col_idx = COLUMN_HEADERS.index("Qty") + 1
                rate_col_idx = COLUMN_HEADERS.index("Rate in Rs") + 1
                labour_amount_col_idx = COLUMN_HEADERS.index(
                    "Labour Amount") + 1
                formula = f"=({
                    get_column_letter(qty_col_idx)}{current_row}*{
                    get_column_letter(rate_col_idx)}{current_row})+{
                    get_column_letter(labour_amount_col_idx)}{current_row}"
                row_values.append(formula)
...
def write_summary_section(
        ...
        data_start_row,
        data_end_row,
        work_subcategory=None):
    """
    Writes the summary section with formulas: Sub Total, GST, Grand Total.

    Note: The allocation code is now static based on work_subcategory.
    Dynamic allocation functionality was removed for code simplification.
    """
    ...
    sub_total_formula = f"=SUM({
        get_column_letter(total_in_rs_col_idx)}{data_start_row}:{
        get_column_letter(total_in_rs_col_idx)}{data_end_row})"

    # Append the row and then set the formula cell type and alignment
    worksheet.append(sub_total_row)
    sub_total_cell = worksheet.cell(
        row=current_row, column=total_in_rs_col_idx)
    ...
    gst_formula = f"={
        get_column_letter(total_in_rs_col_idx)}{sub_total_row_idx}*{GST_RATE}"
    ...
    grand_total_formula = f"={
        get_column_letter(total_in_rs_col_idx)}{sub_total_row_idx}+{
        get_column_letter(total_in_rs_col_idx)}{gst_row_idx}"

    # Append the row and then set the formula cell type and alignment
    worksheet.append(grand_total_row)
    grand_total_cell = worksheet.cell(
        row=current_row, column=total_in_rs_col_idx)
    ...
    # Previously allocated dynamically based on work_subcategory
    if work_subcategory == 'RSP':
        alloc = '47000 050 443 32'
    else:  # M&P or unknown
        alloc = '47000 050 443 32'

    worksheet.cell(row=current_row, column=1, value=f"Allocation : {alloc}")
    current_row += 1

    # Empty Space: Leave 4-5 blank rows for buffer/visual clarity.
    current_row += 5

    # Signature Row: Create four signature placeholders aligned horizontally
    # across the page
